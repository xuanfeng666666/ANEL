...
class StructuralAttentionLayer(nn.Module):
    def __init__(self,
        ...
        self.att_s = nn.Parameter(torch.Tensor(1, n_heads, self.out_dim))
        self.att_r = nn.Parameter(torch.Tensor(1, n_heads, self.out_dim))
        self.att_d = nn.Parameter(torch.Tensor(1, n_heads, self.out_dim))
    ...
    def forward(self, graph, h, h_0, current_node_list):
        ...
        r_type = graph.edata['type']
        H, C = self.n_heads, self.out_dim
        x = self.lin(h).view(-1, H, C)  # [N, heads, out_dim]

        # 直接聚合操作，去除注意力机制
        x_j = x[edge_index[0]]  # [num_edges, heads, out_dim]
        
        # 简单求和聚合，不计算注意力权重
        indices = torch.tensor([current_node_list.index(value) for value in edge_index[1].tolist()], device=h.device)
        out = scatter(x_j, indices, dim=0, reduce="sum")  # [num_nodes, heads, out_dim]

        out = out.reshape(-1, self.n_heads * self.out_dim)  # [num_nodes, output_dim]

        if self.residual:
            # 确保 h_current_node_list_padded 在所有情况下都被定义
            if out.shape[0] < h[current_node_list].shape[0]:
                padding_size = h[current_node_list].shape[0] - out.shape[0]
                out = F.pad(out, (0, 0, 0, padding_size))
                h_current_node_list_padded = h[current_node_list]  # 保持不变
            elif out.shape[0] > h[current_node_list].shape[0]:
                padding_size = out.shape[0] - h[current_node_list].shape[0]
                h_current_node_list_padded = F.pad(h[current_node_list], (0, 0, 0, padding_size))
            else:
                h_current_node_list_padded = h[current_node_list]

            out = out + self.lin_residual(h_current_node_list_padded)

        return out

    # Xavier 初始化仍然保持不变
    def xavier_init(self):
        nn.init.xavier_uniform_(self.att_s)
        nn.init.xavier_uniform_(self.att_d)
