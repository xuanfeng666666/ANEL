...
def test(all_neighbor_list,model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph, mode):
    """
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        ...
        if args.multi_step:
            ...
            if len(predicted_snap):
                ...
                ##############
                neighbor_list.pop(0)
                neighbor_list.append(all_neighbor_list[len(history_list) + time_idx])
...
def run_experiment(args, n_hidden=None, n_layers=None, dropout=None, n_bases=None):
    ...
    model_name = "{}-{}-{}-{}-{}-ly{}-dilate{}-his{}-weight-{}-discount-{}-angle-{}-dp{}-{}-{}-{}-gpu{}"\
        .format(args.dataset, args.basic_model_weight, args.densify_k, args.encoder, args.decoder, args.n_layers, args.dilate_len, args.train_history_len, args.weight, args.discount, args.angle,
    ...
    # create stat
    model = RecurrentRGCN(args.basic_model_weight,
                        args.densify_k,
                        args.decoder,
                        args.encoder,
                        num_nodes,
    ...
    if not os.path.exists("../data/{}/neighbor_list.pkl".format(args.dataset)):
        print("start calculate train neighbors")
        all_neighbor_list = []
        all_glist = [build_sub_graph(num_nodes, num_rels, snap, use_cuda, args.gpu) for snap in train_list]
        for i, g in tqdm(enumerate(all_glist)):
            num_neighbors = torch.tensor([len(set(g.successors(n))) for n in range(g.number_of_nodes())],
                                         dtype=torch.float).cuda()
            all_neighbor_list.append(num_neighbors)
        valid_neighbor_list = []
        valid_glist = [build_sub_graph(num_nodes, num_rels, snap, use_cuda, args.gpu) for snap in valid_list]
        print("start calculate valid neighbors")
        for i, g in tqdm(enumerate(valid_glist)):
            num_neighbors = torch.tensor([len(set(g.successors(n))) for n in range(g.number_of_nodes())],
                                         dtype=torch.float).cuda()
            valid_neighbor_list.append(num_neighbors)
        test_neighbor_list = []
        test_glist = [build_sub_graph(num_nodes, num_rels, snap, use_cuda, args.gpu) for snap in test_list]
        print("start calculate test neighbors")
        for i, g in tqdm(enumerate(test_glist)):
            num_neighbors = torch.tensor([len(set(g.successors(n))) for n in range(g.number_of_nodes())],
                                         dtype=torch.float).cuda()
            test_neighbor_list.append(num_neighbors)
        all_neighbor_list.extend(valid_neighbor_list)
        all_neighbor_list.extend(test_neighbor_list)
        print("finish calculate neighbors")
        with open('../data/{}/neighbor_list.pkl'.format(args.dataset), 'wb') as f:
            pickle.dump(all_neighbor_list, f)
    else:
        with open('../data/{}/neighbor_list.pkl'.format(args.dataset), 'rb') as f:
            all_neighbor_list = pickle.load(f)

    if args.test and os.path.exists(model_state_file):
        mrr_raw, mrr_filter, mrr_raw_r, mrr_filter_r = test(all_neighbor_list,
                                                            model,
        ...
        for epoch in range(args.n_epochs):
            ...
            global_rep = torch.zeros(num_nodes,args.n_hidden).cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--densify_k", type=int, default=10)
    parser.add_argument("--basic_model_weight", type=float, default=0.5)
